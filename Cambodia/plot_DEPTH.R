...
minerogenic <- minerogenic %>% filter(mineral_mass_acc_rate<200)

max_prop <- max(df_prop$prop)
max_mineral <- max(minerogenic$mineral_mass_acc_rate)
scale_factor <- max_prop / max_mineral
plot <- ggplot(df_prop, aes(x = age_median, y = prop, fill = PlantAnimal)) +
  ...
  scale_x_continuous(breaks = centuries,limits=c(900,max(df_prop$age_median))) +
facet_wrap(~site, ncol = 1) + # Arrange facets vertically
  # Add vline and text for events
  geom_vline(
    data = events,
    aes(xintercept = age_median),
    linetype = "dotted",
  ...
  # Add minerogenic data with a secondary y-axis
  geom_line(
    data = minerogenic, 
    aes(x = age_median, y = (mineral_mass_acc_rate) * scale_factor),
    color = "blue",
    linetype = "solid",
    inherit.aes = FALSE
  ) +
  ...
  scale_y_continuous(
    ...
    sec.axis = sec_axis(
      ~ . / scale_factor, 
      name = "Mineral Mass Acc. Rate (g/cm²/yr)"
    )
  ) + ylim(0,1)
  plot
...
pass_plant <- get_top(agg_stats %>% filter(PlantAnimal=="plant" & status=="pass"), 20) %>% filter(total_n_reads >= 20)
plot <- ggplot(pass_plant, aes(x = median_A_b, y = age_median, color = site, size = total_n_reads)) +
  ...
  geom_point(alpha = 0.8) + 
  geom_text(
    data = get_annotations(pass_plant),
    aes(
...
pass_animal <- get_top(agg_stats %>% filter(PlantAnimal=="animal" & status=="pass"), 20) %>% filter(total_n_reads >= 20)
...
plot <- ggplot(pass_animal, aes(x = median_A_b, y = age_median, color = site, size = total_n_reads)) +
  ...
  geom_point(alpha = 0.8) + 
    geom_text(
    data = get_annotations(pass_animal),
    aes(
      label = annotation_text,  
      x = Inf, y = Inf          
    ),
