...
for (i in 1:nrow(df)) {
  ...
  if (!is.na(row_data[,..a_ix]) # data must exist 
    ...
    if (!only5) lines(positions, n_bwd, type = "o", col = "blue")
    legend("right",              
    ...
    plot(positions, f_fwd, type = "o", col = "red", xlab = "Position", ylab = "Damage estimate", ylim = ylim)
    if (!only5) lines(positions, f_bwd, type = "o", col = "blue")  
    lines(positions, x_bwd, type = "l", col = "red")  
    if (!only5) lines(positions, x_bwd, type = "l", col = "blue")  
    legend("topright",              
